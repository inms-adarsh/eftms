...
function can_process($myrow)
{
    ...
    if (!has_bom($myrow['stock_id'],$myrow['vehicle_id']) && $myrow['type'] != WO_ADVANCED)
	{
...
{
	release_work_order($selected_id,$_POST['odometer_reading'], $_POST['released_date'], $_POST['memo_']);
...
if (can_process($myrow))
{
	...
	$result= get_odometer_reading_unit($myrow['vehicle_id']);
