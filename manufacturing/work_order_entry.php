...
check_db_has_manufacturable_items(_("There are no services defined in the system."));

check_db_has_locations(("There are no locations defined in the system."));
...
{
	$selected_id2 = $_GET['trans_no'];
}
elseif(isset($_POST['selected_id2']))
{
	$selected_id2 = $_POST['selected_id2'];
}
...
{
	global $selected_id2, $SysPrefs, $Refs;

	if (!isset($selected_id2))
	{
	...
	if (!get_post('stock_id')) 
	 {
		display_error(_("This is no service selected."));
		set_focus('stock_id');
		return false;
	} 
	...
	{
        if (!has_bom($_POST['stock_id'],$_POST['vehicle_id']))
        {
	...
	if (input_num('odometer_reading')==0)
    	{
    		display_error( _("The reading entered is invalid or zero."));
			set_focus('odometer_reading');
    		return false;
    	}
        ...
        if (!$SysPrefs->allow_negative_stock())
        {
        	if ($_POST['type'] == WO_ASSEMBLY)
        	{
                ...
                $result = get_bom($_POST['stock_id'],$_POST['vehicle_id']);
     ...
     else
     {
    	...
    	if (isset($selected_id2))
    	{
    		$myrow = get_work_order($selected_id2, true);
...
if (isset($_POST['ADD_ITEM2']) && can_process())
{
	if (!isset($_POST['cr_acc']))
		$_POST['cr_acc'] = "";
	if (!isset($_POST['cr_lab_acc']))
		$_POST['cr_lab_acc'] = "";
	$id = add_work_order($_POST['wo_ref'],$_POST['odometer_reading'], $_POST['StockLocation'], input_num('quantity'),
		$_POST['stock_id'], $_POST['vehicle_id'], $_POST['type'], $_POST['date_'],
...
if (isset($_POST['UPDATE_ITEM2']) && can_process())
{

	update_work_order($selected_id2,$_POST['odometer_reading'], $_POST['StockLocation'], input_num('quantity'),
		$_POST['stock_id'], $_POST['vehicle_id'], $_POST['date_'], $_POST['RequDate'], $_POST['memo_']);
	new_doc_date($_POST['date_']);
	meta_forward($_SERVER['PHP_SELF'], "UpdatedID=$selected_id2");
}
...
if (isset($_POST['delete']))
{
	...
	if (work_order_has_productions($selected_id2) ||
		work_order_has_issues($selected_id2)	||
		work_order_has_payments($selected_id2))
	{
	...
	{ //ie not cancelled the delete as a result of above tests
		...
		delete_work_order($selected_id2);
		meta_forward($_SERVER['PHP_SELF'], "DeletedID=$selected_id2");
	}
}
...
if (isset($_POST['close']))
{
	...
	close_work_order($selected_id2);
	meta_forward($_SERVER['PHP_SELF'], "ClosedID=$selected_id2");
}
...
if (isset($selected_id2))
{
	$myrow = get_work_order($selected_id2);
	...
	$_POST['memo_'] = get_comments_string(ST_WORKORDER, $selected_id2);
...
vehicle_list_row(_("Vehicle:"), 'vehicle_id', null, false, true, false, true);
start_row();
service_item_list_row(_("Select a Service:"), 'stock_id', null, false, true);
end_row();
...
if (isset($selected_id2))
{
	...
	submit_cells('UPDATE_ITEM2', _("Update"), '', _('Save changes to work order'), 'default');
	if (get_post('released'))
	{
...
{
	submit_center('ADD_ITEM2', _("Add Workorder"), true, '', 'default');
}
...
{
	$result = get_bom($selected_parent,$_POST['vehicle_id']);
	div_start('bom');
	display_heading("Define Bill of Materials");
	start_table(TABLESTYLE, "width=60%");
	$th = array(_("Vehicle"),_("Code"), _("Description"), _("Location"),
...
function on_submit($selected_parent, $selected_component=-1)
{
	...
	else
	{
		...
		if (!check_for_recursive_bom($selected_parent, $_POST['component']))
		{
			...
			if (!is_component_already_on_bom($_POST['component'], $_POST['vehicle_id'], $_POST['workcentre_added'],
				$_POST['loc_code'], $selected_parent))
			{
				add_bom($selected_parent, $_POST['component'], $_POST['workcentre_added'], $_POST['vehicle_id'],
					$_POST['loc_code'], input_num('quantities'));
...
start_form();
	...
	if ($selected_id != -1)
	{
 		if ($Mode == 'Edit') {
			...
			$_POST['quantities'] = number_format2($myrow["quantity"], get_qty_dec($myrow["component"]));
	...
	$_POST['quantities'] = number_format2(input_num('quantities',1), $dec);
	qty_row(_("Quantity:"), 'quantities', null, null, null, $dec);
