...
function can_process()
{
	...
	if (!is_tax_gl_unique(get_post('transport_gl_code'), get_post('purchasing_gl_code'), $selected_id)) {
		display_error( _("Selected GL Accounts cannot be used by another tax type."));
		set_focus('transport_gl_code');
		return false;
	}
	return true;
}
...
if ($Mode=='ADD_ITEM' && can_process())
{
	...
	add_tax_type($_POST['name'], $_POST['transport_gl_code'],
...
if ($Mode=='UPDATE_ITEM' && can_process())
{
	...
	update_tax_type($selected_id, $_POST['name'],
    	$_POST['transport_gl_code'], $_POST['purchasing_gl_code'], input_num('rate'));
...
display_note(_("To avoid problems with manual journal entry all tax types should have unique Transport/Purchasing GL accounts."), 0, 1);
start_table(TABLESTYLE);

$th = array(_("Description"), _("Default Rate (%)"),
	_("Transport GL Account"), _("Purchasing GL Account"), "", "");
...
while ($myrow = db_fetch($result))
{
	...
	label_cell($myrow["transport_gl_code"] . "&nbsp;" . $myrow["TransportAccountName"]);
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$_POST['transport_gl_code']  = $myrow["transport_gl_code"];
...
gl_all_accounts_list_row(_("Transport GL Account:"), 'transport_gl_code', null);
