...
// Author: Joe Hunt, 11/06/2011
	...
	class renderer
	{
		...
		function menu_header($title, $no_menu, $is_index)
		{
			...
			echo "<a href='javascript:;'><img src=$logo alt='' height='100%'/></a>";
			echo "			</div>";
			echo "			<div class='info'>";
			echo $_SESSION["wa_current_user"]->name;
			echo "				<small>".$db_connections[$_SESSION["wa_current_user"]->company]["name"]."</small>";
			echo "			</div>";
			echo "		</li>";
			echo "	</ul>";
			echo "	<!-- end sidebar user -->";
			echo "	<!-- begin sidebar nav -->";
			echo "<ul class='nav'>\n";
				...
				foreach($applications as $app)
				{
					 ...
					 if ($_SESSION["wa_current_user"]->check_application_access($app))
                    {
						...
						$acc = access_string($app->name);
						$i++;
				/*		echo "<li ".($account[3] == $app->id ? "class='has-sub active' " : "class='has-sub'") . "><a href='javascript:;'><b class='caret pull-right'></b><i class='fa fa-file-o'></i><span>" . $acc[0] . "</span></a>\n";
					...
					*/foreach ($app->modules as $module)
					{
						...
						foreach ($module->rappfunctions as $appfunction)
							$apps[] = $appfunction;
						echo "<li class='has-sub'><a href='#'><b ".(count($apps) ? "class='caret pull-right'></b>":"")."$module->name</a>\n"; 
						$application = array();	
						if (count($apps))
							echo "<ul class='sub-menu'>\n";	
						foreach ($apps as $application)	
						{
							...
							elseif (!$_SESSION["wa_current_user"]->hide_inaccessible_menu_items())	
								echo "<li><a href='#' style='color:gray;'>{$lnk[0]}</a></li>\n";
						}
						$j++;
						if (count($apps))
							echo "</ul>\n";	
		...
		echo "<!-- end #sidebar -->";
		 ...
		 if ($no_menu)
			...
			{
				echo " <!-- begin breadcrumb -->";
		 		echo "	<div class='breadcrumb pull-right'>";
				echo "	".(user_hints() ? "<span id='hints'></span>" : '')."";
		 		echo "	</div>";
		 		echo "	<!-- end breadcrumb -->";
		 		echo "	<!-- begin page-header -->";
		 		echo "	<div class='page-header pull-left'>$title</div>";
		...
		function display_applications(&$waapp)
		{
			...
			elseif ($selected_app->id == "orders")
				include($path_to_root . "/includes/page/home.inc");
