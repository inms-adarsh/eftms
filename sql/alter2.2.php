...
class fa2_2 {
	...
	function install($pref, $force) 
	{
		...
		$sql = "UPDATE {$pref}stock_category SET "
			."dflt_transport_act = '" . $prefs['default_inv_transport_act'] . "',"
		...
		if (!($ret = db_query("SELECT MAX(`order_no`) FROM `{$pref}transport_orders`")) ||
			!db_num_rows($ret))
		{
				display_error(_('Cannot query max transport order number.'));
				return false;
		} 
		$row = db_fetch($ret);
		$max_order = $row[0];
		...
		if(!db_query($sql))
		{
			display_error(_('Cannot store next transport order reference.'));
			return false;
		}
		...
		if (!$this->beta) {
			$n = 16;
 			if (check_table($pref, 'company')) // skip in 2.3
 				...
 				if (check_table($pref, 'company', 'consignorom1_name')) $patchcnt++;
			...
			if (!check_table($pref, 'consignor_branch', 'inactive')) $patchcnt++;
			...
			if (!check_table($pref, 'transport_orders', 'reference')) $patchcnt++;
...
function convert_roles($pref) 
{
	...
	$trans_sec = array(
		1 => array('SA_CHGPASSWD', 'SA_SETUPDISPLAY', 'SA_BANKTRANSVIEW',
			'SA_ITEMSTRANSVIEW','SA_SUPPTRANSVIEW', 'SA_TRANSPORTORDER',
			'SA_TRANSPORTALLOC', 'SA_TRANSPORTTRANSVIEW'),
		2 => array('SA_DIMTRANSVIEW', 'SA_STANDARDCOST', 'SA_ITEMSTRANSVIEW',
			'SA_ITEMSSTATVIEW', 'SA_TRANSPORTPRICE', 'SA_MANUFTRANSVIEW',
			'SA_WORKORDERANALYTIC', 'SA_WORKORDERCOST', 'SA_SUPPTRANSVIEW',
			'SA_SUPPLIERALLOC', 'SA_STEMPLATE', 'SA_TRANSPORTTRANSVIEW',
			'SA_TRANSPORTINVOICE', 'SA_TRANSPORTDELIVERY', 'SA_CONSIGNORPAYMREP',
			'SA_CONSIGNORBULKREP', 'SA_PRICEREP', 'SA_TRANSPORTBULKREP', 'SA_TRANSPORTMANREP',
			'SA_TRANSPORTBULKREP', 'SA_CONSIGNORSTATREP', 'SA_SUPPLIERANALYTIC',
			...
			'SA_GLANALYTIC', 'SA_TAXREP', 'SA_TRANSPORTANALYTIC', 'SA_TRANSPORTQUOTE'),
		3 => array('SA_GLACCOUNTGROUP', 'SA_GLACCOUNTCLASS','SA_PAYMENT', 
			...
			'SA_MANUFISSUE', 'SA_SUPPLIERALLOC', 'SA_CONSIGNOR', 'SA_CRSTATUS',
			'SA_TRANSPORTMAN', 'SA_TRANSPORTAREA', 'SA_TRANSPORTALLOC', 'SA_TRANSPORTCREDITINV',
			'SA_TRANSPORTPAYMNT', 'SA_TRANSPORTCREDIT', 'SA_TRANSPORTGROUP', 'SA_SRECURRENT',
		...
		11 => array('SA_ITEMCATEGORY', 'SA_ITEM', 'SA_UOM', 'SA_INVENTORYLOCATION',
			 'SA_GRN', 'SA_FORITEMCODE', 'SA_TRANSPORTKIT'),
		14 => array('SA_SHIPPING', 'SA_VOIDTRANSACTION', 'SA_TRANSPORTTYPES'),
