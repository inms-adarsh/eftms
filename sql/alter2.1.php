...
class fa2_1 {
	...
	*/
		...
		db_query($sql);
		//
		//	Move all consignor and supplier trans tax details to new table
		...
		//
	$move_sql =array( 
	"consignor_trans_tax_details" =>
		"SELECT tr.tran_date, tr.type, tr.trans_no, dt.tax_type_id, 
			dt.rate, dt.included_in_price, dt.amount, tr.reference as ref,
			tr.rate as ex_rate
		FROM ".$pref."consignor_trans_tax_details dt	
			LEFT JOIN ".$pref."trans_tax_details tt
				ON dt.consignor_trans_no=tt.trans_no 
				AND dt.consignor_trans_type=tt.trans_type,
			".$pref."consignor_trans tr
		WHERE tt.trans_type is NULL
			AND dt.consignor_trans_no = tr.trans_no 
			AND dt.consignor_trans_type = tr.type",
	...
	// trans tax details tables was deleted during previous try.  
		if (check_table($pref, 'consignor_trans_tax_details') 
