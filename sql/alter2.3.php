...
class fa2_3 {
	...
	function install($pref, $force) 
	{
		...
		if (!$this->beta) {
			...
			$sql = "SELECT consignor_no, payment_terms FROM {$pref}consignors_master";
		
			$result = db_query($sql);
			if (!$result) {
				display_error("Cannot read consignors"
				.':<br>'. db_error_msg($db));
				return false;
			}
			// update all transport orders and transactions with consignor std payment terms
			while($consignor = db_fetch($result)) {
				$sql = "UPDATE {$pref}consignor_trans SET "
					."payment_terms = '" .$consignor['payment_terms']
					."' WHERE consignor_no='".$consignor['consignor_no']."'";
				if (db_query($sql)==false) {
					display_error("Cannot update consignor trans payment"
					.':<br>'. db_error_msg($db));
					return false;
				}
				$sql = "UPDATE {$pref}transport_orders SET "
					."payment_terms = '" .$consignor['payment_terms']
					."' WHERE consignor_no='".$consignor['consignor_no']."'";
				if (db_query($sql)==false) {
					display_error("Cannot update transport order payment"
					.':<br>'. db_error_msg($db));
					return false;
				}
			}
			...
			if (!$this->update_line_relations($pref)) {
				display_error("Cannot update transport document links");
				return false;
			}
			...
			$dropcol = array(
				...
				'consignors_master' => array('email'),
				'consignor_branch' => array('phone', 'phone2', 'fax', 'email'),
				'suppliers' => array('phone', 'phone2', 'fax', 'email'),
				'consignor_trans' => array('trans_link')
			);
	...
	function installed($pref) {
		...
		if (!$this->beta) {
			...
			if (!check_table($pref, 'transport_orders', 'payment_terms')) $patchcnt++;
		}
	...
	*/
	function update_totals($pref)
	{
		...
		$cart = new cart(ST_TRANSPORTORDER);
		$sql = "SELECT order_no, trans_type FROM {$pref}transport_orders";
		$orders = db_query($sql);
		if (!$orders)
			return false;
		while ($order = db_fetch($orders)) {
			read_transport_order($order['order_no'], $cart, $order['trans_type']);
			$result = db_query("UPDATE {$pref}transport_orders 
	...
	function get_parent_trans_2_2($trans_type, $trans_no) {
		...
		$sql = 'SELECT trans_link FROM
				'.TB_PREF.'consignor_trans WHERE trans_no='.db_escape($trans_no)
		...
		if ($trans_type!=ST_TRANSPORTINVOICE) return 0;	// this is credit note with no parent invoice
		// invoice: find batch invoice parent trans.
		$sql = 'SELECT trans_no FROM
				'.TB_PREF.'consignor_trans WHERE
	...
	/*
		...
		there can be transport documents with lines not properly linked to parents. This rare 
	...
	*/
	function update_line_relations($pref)
	{
		...
		$sql =	"SELECT d.type, trans_no, order_ FROM {$pref}consignor_trans d
			LEFT JOIN {$pref}voided v ON d.type=v.type AND d.trans_no=v.id
				WHERE ISNULL(v.type) AND 
				(d.type=".ST_CONSIGNORDELIVERY
				." OR d.type=".ST_TRANSPORTINVOICE
				." OR d.type=".ST_CONSIGNORCREDIT.")";
		$result = db_query($sql);
		if (!$result)
			return false;
		...
		while ($trans = db_fetch($result)) {
			...
			$lines = get_consignor_trans_details($type, $trans_no);
			$n = db_num_rows($lines);

			if ($type==ST_CONSIGNORDELIVERY)
				$src_lines = get_transport_order_details($trans['order_'], ST_TRANSPORTORDER);
			else
				$src_lines =  get_consignor_trans_details(get_parent_type($type), 
					$this->get_parent_trans_2_2($type, $trans_no));

			$src_n = db_num_rows($src_lines);

			if (($type == ST_CONSIGNORCREDIT) && ($src_n == 0))
				 continue;  // free credit note has no src lines 

			$max = $type == ST_CONSIGNORDELIVERY ? $n : max($src_n, $n);
			...
			for($i = 0, $j=0; $i < $max; $i++) {
				...
				if ($src_line['stock_id'] == $doc_line['stock_id']
					...
					$sql = "UPDATE {$pref}consignor_trans_details SET src_id = {$src_line['id']}
