...
class manufacturing_app extends application
{
	...
	{
		$this->application("manuf", _($this->help_context = "&Maintenance"));
		...
		$this->add_rapp_function(1, _("Work Order &Inquiry"),
			"manufacturing/search_work_orders.php?", 'SA_MANUFTRANSVIEW', MENU_INQUIRY);
		$this->add_rapp_function(1, _("Maintenance &Reports"),
			"reporting/reports_main.php?Class=3", 'SA_MANUFTRANSVIEW', MENU_REPORT);

		
		
		$this->add_module(_("Servicing Details"));
	$this->add_lapp_function(2, _("Services"),
			"inventory/manage/services.php?", 'SA_ITEM', MENU_ENTRY);
		$this->add_rapp_function(2, _("&Work Centres"),
			"manufacturing/manage/work_centres.php?", 'SA_WORKCENTRES', MENU_MAINTENANCE);
		$this->add_module(_("Inventory"));
		
			$this->add_lapp_function(3, _("&Parts"),
			"inventory/manage/parts_materials.php?", 'SA_ITEM', MENU_ENTRY);
			$this->add_lapp_function(3, _("&Fuels"),
			"inventory/manage/materials.php?", 'SA_ITEM', MENU_ENTRY);
				
		$this->add_lapp_function(3, _("Item &Categories"),
			"inventory/manage/item_categories.php?", 'SA_ITEMCATEGORY', MENU_MAINTENANCE);
	
		$this->add_rapp_function(3, _("Inventory &Movement Types"),
			"inventory/manage/movement_types.php?", 'SA_INVENTORYMOVETYPE', MENU_MAINTENANCE);
		
		$this->add_rapp_function(3, _("&Reorder Levels"),
		...
		$this->add_rapp_function(3, _("Inventory &Reports"),
