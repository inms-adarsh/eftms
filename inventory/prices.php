...
***********************************************************************/
$page_security = 'SA_TRANSPORTPRICE';
if (!@$_GET['popup'])
	$path_to_root = "..";
else	
	$path_to_root = "../..";
...
if (!@$_GET['popup'])
	page(_($help_context = "Inventory Item Transport prices"));

//---------------------------------------------------------------------------------------------------

check_db_has_stock_items(_("There are no items defined in the system."));

check_db_has_transport_types(_("There are no transport types in the system. Please set up transport types befor entering pricing."));
...
{
	echo "<center>" . _("Item:"). "&nbsp;";
	echo transport_items_list('stock_id', $_POST['stock_id'], false, true, '', array('editable' => false));
	echo "<hr></center>";
}
else
	br(2);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if (!check_num('price', 0))
	{
   	...
   	elseif ($Mode == 'ADD_ITEM' && get_stock_price_type_currency($_POST['stock_id'], $_POST['transport_type_id'], $_POST['curr_abrev']))
   	{
      	$input_error = 1;
      	display_error( _("The transport pricing for this item, transport type and currency has already been added."));
	...
	if ($input_error != 1)
	{
    	...
    	if ($selected_id != -1) 
		{
			...
			update_item_price($selected_id, $_POST['transport_type_id'],
		...
		else
		{
			...
			add_item_price($_POST['stock_id'], $_POST['transport_type_id'],
			    $_POST['curr_abrev'], input_num('price'));
...
if (list_updated('stock_id') || isset($_POST['_curr_abrev_update']) || isset($_POST['_transport_type_id_update'])) {
	// after change of stock, currency or transporttype selector
...
$th = array(_("Currency"), _("Transport Type"), _("Price"), "", "");
table_header($th);
...
if ($Mode == 'Edit')
{
	...
	$_POST['transport_type_id'] = $myrow["transport_type_id"];
...
transport_types_list_row(_("Transport Type:"), 'transport_type_id', null, true);

if (!isset($_POST['price'])) {
	$_POST['price'] = price_format(get_kit_price(get_post('stock_id'), 
		get_post('curr_abrev'),	get_post('transport_type_id')));
}
