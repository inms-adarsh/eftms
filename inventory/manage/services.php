...
page(_($help_context = "Services"), @$_REQUEST['popup'], false, "", $js);
...
if (isset($_FILES['pic']) && $_FILES['pic']['name'] != '') 
{
	...
	$Ajax->activate('details');
 /* EOF Add Image upload for New Service  - by Ori */
}
...
if (isset($_POST['addupdate'])) 
{
	...
	if ($input_error != 1)
	{
		...
		{ /*so its an existing one */
			update_service($_POST['NewStockID'],$_POST['service_quantity'],$_POST['alarm_quantity'],$_POST['time_id'], 
			$_POST['alarm_time_id'],$_POST['description'],
			...
			display_notification(_("Service has been updated."));
		} 
		else 
		{ //it is a NEW part

			add_service($_POST['NewStockID'],$_POST['service_quantity'],$_POST['alarm_quantity'],$_POST['time_id'], 
			$_POST['alarm_time_id'], $_POST['description'],	$_POST['long_description'], $_POST['category_id'], $_POST['tax_type_id'],
...
function item_settings(&$stock_id) 
{
	...
	table_section_title(_("Service"));

	//------------------------------------------------------------------------------------
	if ($new_item) 
	{
		text_row(_("Service Code:"), 'NewStockID', null, 21, 20);
		$_POST['service_quantity'] = '';
		$_POST['alarm_quantity'] = '';
$_POST['time_id']  = '';
			$_POST['alarm_time_id']  = '';
		$_POST['inactive'] = 0;
	} 
	...
	{ // Must be modifying an existing item
		if (get_post('NewStockID') != get_post('stock_id') || get_post('addupdate')) { // first item display
			...
			$_POST['service_quantity']  = $myrow["service_quantity"];
			$_POST['alarm_quantity']  = $myrow["alarm_quantity"];
			$_POST['time_id']  = $myrow["time_id"];
			$_POST['alarm_time_id']  = $myrow["alarm_time_id"];
		...
		label_row(_("Service Code:"),$_POST['NewStockID']);
	...
	stock_categories_service_list_row(_("Category:"), 'category_id', null, false, $new_item);
...
$_POST['tax_type_id'] = $category_record["dflt_tax_type"];
	...
	item_tax_types_list_row(_("Service Tax Type:"), 'tax_type_id', null);
	 qty_row(_("Repeat Every:"), 'service_quantity', null, null, null, 0);
	stock_time_types_list_row('', 'time_id', $_POST['time_id']);
	 qty_row(_("Alarm Before:"), 'alarm_quantity', null, null, null, 0);
	stock_time_types_list_row('', 'alarm_time_id', $_POST['alarm_time_id']);
	hidden('mb_flag', "M");
	...
	if (is_manufactured($_POST['mb_flag']))
		gl_all_accounts_list_row(_("Service Assembly Costs Account:"), 'assembly_account', $_POST['assembly_account']);
	else
		hidden('assembly_account', $_POST['assembly_account']);

	table_section_title(_("Other"));

	// Add image upload for New Service  - by Joe
	...
	record_status_list_row(_("Service status:"), 'inactive');
	end_outer_table(1);

	div_start('controls');
	if (!isset($_POST['NewStockID']) || $new_item) 
	{
		submit_center('addupdate', _("Insert New Service"), true, '', 'default');
	} 
	else 
	{
		submit_center_first('addupdate', _("Update Service"), '', 
		...
		submit('clone', _("Clone This Service"), true, '', true);
		submit('delete', _("Delete This Service"), true, '', true);
...
if (db_has_stock_items()) 
{
	...
	start_row();
    service_item_list_cells(_("Select an service:"), 'stock_id', null,
	  _('New service'), true, check_value('show_inactive'));
...
tabbed_content_start('tabs', array(
	...
	//	'bill_of_ materials' => array(_('&Bill of Materials'), $stock_id),
	));
	...
	switch (get_post('_tabs_sel')) {
		...
		case 'bill_of_materials':
			$_GET['stock_id'] = $stock_id;
			$_GET['popup'] = 1;
			include_once($path_to_root."/inventory/manage/bom_edit.php");
			break;
	};
br();
tabbed_content_end();
