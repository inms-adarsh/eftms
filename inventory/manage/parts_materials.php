...
page(_($help_context = "Parts"), @$_REQUEST['popup'], false, "", $js);
...
if (isset($_FILES['pic']) && $_FILES['pic']['name'] != '') 
{
	...
	$Ajax->activate('details');
 /* EOF Add Image upload for New Part  - by Ori */
}
...
if (isset($_POST['addupdate'])) 
{
	...
	if ($input_error != 1)
	{
		...
		{ /*so its an existing one */
			update_stock_part($_POST['NewStockID'],$_POST['part_type_id'], $_POST['description'],
			...
			display_notification(_("Part has been updated."));
		} 
		else 
		{ //it is a NEW part

			add_stock_part($_POST['NewStockID'],$_POST['part_type_id'], $_POST['description'],
...
function item_settings(&$stock_id) 
{
	...
	table_section_title(_("Part"));

	//------------------------------------------------------------------------------------
	if ($new_item) 
	{
		text_row(_("Part Code:"), 'NewStockID', null, 21, 20);
		$_POST['part_type_id'] = '';
	...
	{ // Must be modifying an existing item
		if (get_post('NewStockID') != get_post('stock_id') || get_post('addupdate')) { // first item display
			...
			$_POST['part_type_id']  = $myrow["part_type_id"];
		...
		label_row(_("Part Code:"),$_POST['NewStockID']);
	...
	stock_categories_component_list_row(_("Category:"), 'category_id', null, false, $new_item);
...
$_POST['tax_type_id'] = $category_record["dflt_tax_type"];
	...
	item_tax_types_list_row(_("Part Tax Type:"), 'tax_type_id', null);
	part_categories_list_row(_("Part Type:"), 'part_type_id', $_POST['part_type_id']);
	hidden('mb_flag', "B");
	...
	if (is_manufactured($_POST['mb_flag']))
		gl_all_accounts_list_row(_("Part Assembly Costs Account:"), 'assembly_account', $_POST['assembly_account']);
	else
		hidden('assembly_account', $_POST['assembly_account']);

	table_section_title(_("Other"));

	// Add image upload for New Part  - by Joe
	...
	record_status_list_row(_("Part status:"), 'inactive');
	end_outer_table(1);

	div_start('controls');
	if (!isset($_POST['NewStockID']) || $new_item) 
	{
		submit_center('addupdate', _("Insert New Part"), true, '', 'default');
	} 
	else 
	{
		submit_center_first('addupdate', _("Update Part"), '', 
		...
		submit('clone', _("Clone This Part"), true, '', true);
		submit('delete', _("Delete This Part"), true, '', true);
...
if (db_has_stock_items()) 
{
	...
	start_row();
    component_item_list_cells(_("Select an part:"), 'stock_id', null,
	  _('New part'), true, check_value('show_inactive'));
