...
if (!@$_GET['popup'])
	page(_($help_context = "Vendor Purchasing Data"));

check_db_has_purchasable_items(_("There are no purchasable transportable items defined in the system."));
     	...
     	if ($Mode == 'ADD_ITEM') 
       	...
       	else
       	{
       		update_item_purchasing_data($selected_id, $_POST['stock_id'], input_num('price',0),
       			$_POST['suppliers_uom'], input_num('conversion_factor'), $_POST['supplier_description']);
    	  	display_notification(_("Vendor purchasing data has been updated."));
       	}
		$Mode = 'RESET';
	}
}
...
else
{
    ...
    else
    {
		...
		$th = array(_("Vendor"), _("Price"), _("Currency"),
			_("Vendor's Unit"), _("Conversion Factor"), _("Vendor's Description"), "", "");
...
{
	hidden('supplier_id');
	label_row(_("Vendor:"), $supp_name);
}
else
{
	supplier_list_row(_("Vendor:"), 'supplier_id', null, false, true);
...
text_row(_("Vendors Unit of Measure:"), 'suppliers_uom', null, 50, 51);
...
text_row(_("Vendor's Code or Description:"), 'supplier_description', null, 50, 51);
