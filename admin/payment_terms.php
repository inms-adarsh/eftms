...
if ($Mode == 'Delete')
{
	...
	if (key_in_foreign_table($selected_id, 'consignors_master', 'payment_terms'))
	{
		display_error(_("Cannot delete this payment term, because consignor accounts have been created referring to this term."));
	} 
	else 
	{
		...
		{
			//only delete if used in neither consignor or supplier accounts
			delete_payment_terms($selected_id);
			display_notification(_('Selected payment terms have been deleted'));
		}
	}
	//end if payment terms used in consignor or supplier accounts
	$Mode = 'RESET';
}
...
text_row_ex(_("Terms Description:"), 'terms', null, 40, 40);
