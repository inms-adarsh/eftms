...
check_row(_("Show inactive:"), 'show_inactive', null, true);
...
start_table(TABLESTYLE2);
	text_row_ex(_("Role name:"), 'name', null, 20, 22);
	text_row_ex(_("Role description:"), 'description', null, 50, 52);
	record_status_list_row(_("Current status:"), 'inactive');
end_table(1);

	start_display_table('table table-bordered','access');
...
//			 display_error(sprintf("Bad section %X:", $m));
			label_row_table($security_sections[$m].':', 
		...
		if (check_value('Section'.$m)) {
				...
				check_table_cells($parms[1], 'Area'.$parms[0], null, 
					false, '', "align='center'");
