...
function exist_transaction($type, $type_no)
{
	...
	switch ($type) 
	{
		...
		case ST_TRANSPORTINVOICE : // it's a consignor invoice
		case ST_CONSIGNORCREDIT : // it's a consignor credit note
		case ST_CONSIGNORPAYMENT : // it's a consignor payment
		case ST_CONSIGNORDELIVERY : // it's a consignor dispatch
			if (!exists_consignor_trans($type, $type_no))
				return false;
			break;
		...
		case ST_TRANSPORTORDER: // it's a transport order
		case ST_TRANSPORTQUOTE: // it's a transport quotation
		case ST_TRANSPORTBOOKING: // it's a transport booking
			return false;
...
function voiding_controls()
{
	...
	$not_implemented =  array(ST_PURCHORDER, ST_TRANSPORTORDER, ST_TRANSPORTQUOTE, ST_TRANSPORTBOOKING,ST_COSTUPDATE);
	...
	systypes_list_row(_("Type:"), 'filterType', null, true, $not_implemented);
    ...
    ref_row(_("from #:"), 'FromTransNo');

    ref_row(_("to #:"), 'ToTransNo');
