...
if ($Mode == 'Delete')
{
// PREVENT DELETES IF DEPENDENT RECORDS IN 'transport_orders'

	if (key_in_foreign_table($selected_id, 'transport_orders', 'ship_via'))
	{
		$cancel_delete = 1;
		display_error(_("Cannot delete this shipping company because transport orders have been created using this shipper."));
	} 
	else 
	{
		// PREVENT DELETES IF DEPENDENT RECORDS IN 'consignor_trans'
		if (key_in_foreign_table($selected_id, 'consignor_trans', 'ship_via'))
		{
			$cancel_delete = 1;
