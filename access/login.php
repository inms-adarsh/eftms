...
function defaultCompany()
{
	...
	echo "<head profile=\"http://www.w3.org/2005/10/profile\"><title>$title</title>\n";
   	...
   	echo "<link href='$path_to_root/themes/fa-boot/default.css' rel='stylesheet' type='text/css'> \n";
	echo "<link href='$path_to_root/themes/coloradmin/assets/css/style.css' rel='stylesheet' type='text/css'> \n";
 	echo "<link href='$path_to_root/themes/fa-boot/images/favicon.PNG' rel='icon' type='image/x-icon'> \n";
	send_scripts();
	if (!$login_timeout)
	{
		echo $js;
	...
	if (!$login_timeout) { // FA logo
    	echo "<a target='_blank' href='$power_url'><img src='$path_to_root/themes/fa-boot/images/logo_frontaccounting.png' alt='FrontAccounting' height='50' onload='fixPNG(this)' border='0' /></a>";
	} else { 
	...
	if (!$login_timeout)
		form_section_title(_("Version")." $version   Build $build_version - "._("Login"));
	$value = $login_timeout ? $_SESSION['wa_current_user']->loginname : ($allow_demo_mode ? "demouser":"");

	text_row_ex(_("User name"), "user_name_entry_field", $value, 20, 30);
	...
	if ($login_timeout) {
		...
		if (!@$text_company_selection) {
			echo "<div class='form-group'><label class='col-md-3 control-label'>"._("Company")."</label><div class='col-md-6'><select class='form-control' name='company_login_name'>\n";
...
//			$coy = $def_coy;
			text_row_ex(_("Company"), "company_login_nickname", "", 20, 50);
		}
		start_row();
