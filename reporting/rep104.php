...
function print_price_listing()
{
	...
	if ($transporttype == 0)
		...
		$stype = get_transport_type_name($transporttype);
	if ($showGP == 0)
		$GP = _('No');
	else
		$GP = _('Yes');
    ...
    $params =   array( 	0 => $comments,
    				    ...
    				    3 => array('text' => _('Transport Type'), 'from' => $stype, 'to' => ''),
	...
	$catgor = '';
	$_POST['transport_type_id'] = $transporttype;
	while ($myrow=db_fetch($result))
	{
		...
		$price = get_price($myrow['stock_id'], $currency, $transporttype);
		$rep->AmountCol(3, 4, $price, $dec);
		if ($showGP)
		{
			$price2 = get_price($myrow['stock_id'], $home_curr, $transporttype);
			if ($price2 != 0.0)
	...
	while ($myrow=db_fetch($result))
	{
		if ($catgor != $myrow['cat_name'])
		{
			if ($catgor == '')
			{
				$rep->NewLine(2);
				$rep->fontSize += 2;
				$rep->TextCol(0, 3, _("Transport Kits"));
				$rep->fontSize -= 2;
			}
			$rep->Line($rep->row  - $rep->lineHeight);
			$rep->NewLine(2);
		...
		$price = get_kit_price($myrow['kit_code'], $currency, $transporttype);
		$rep->AmountCol(3, 4, $price, $dec);
		$rep->NewLine(0, 1);
	}
	$rep->Line($rep->row  - 4);
