...
function print_inventory_planning()
{
	...
	while ($trans=db_fetch($res))
	{
		...
		if ($location == 'all')
			...
			$loc_code = $location;
		$consignorqty = get_demand_qty($trans['stock_id'], $loc_code);
		$consignorqty += get_demand_asm_qty($trans['stock_id'], $loc_code);
		...
		$MaxMthTransport = Max($period['prd0'], $period['prd1'], $period['prd2'], $period['prd3']);
		$IdealStockHolding = $MaxMthTransport * 3;
		$rep->AmountCol(7, 8, $IdealStockHolding, $dec);

		$rep->AmountCol(8, 9, $trans['qty_on_hand'], $dec);
		$rep->AmountCol(9, 10, $consignorqty, $dec);
		$rep->AmountCol(10, 11, $suppqty, $dec);

		$SuggestedTopUpOrder = $IdealStockHolding - $trans['qty_on_hand'] + $consignorqty - $suppqty;
