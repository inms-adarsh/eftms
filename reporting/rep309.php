...
function getTransactions($category, $from, $to)
{
	...
	$sql = "SELECT item.category_id,
		...
		FROM ".TB_PREF."stock_master item,
			...
			".TB_PREF."consignor_trans trans,
			".TB_PREF."consignor_trans_details line
		...
		AND line.quantity<>0
		AND line.consignor_trans_type = ".ST_TRANSPORTINVOICE;
		if ($category != 0)
...
function print_inventory_transport()
{
	...
	$headers = array(_('Item/Category'), _('Description'), _('Qty'), _('Unit Price'), _('Transport'), '', _('Remark'));	
    ...
    $rep = new FrontReport(_('Item Transport Summary Report'), "ItemTransportSummaryReport", user_pagesize(), 9, $orientation);
    if ($orientation == 'L')
