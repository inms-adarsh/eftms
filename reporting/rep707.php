...
function print_profit_and_loss_statement()
{
	...
	while ($class = db_fetch($classresult))
	{
		...
		$transportper += $class_per_total;
		$transportacc += $class_acc_total;
	}
	
	$rep->Font('bold');	
	$rep->TextCol(0, 2,	_('Calculated Return'));
	$rep->AmountCol(2, 3, $transportper *-1, $dec); // always convert
	$rep->AmountCol(3, 4, $transportacc * -1, $dec);
	$rep->AmountCol(4, 5, Achieve($transportper, $transportacc), $pdec);
	if ($graphics)
	{
		$pg->x[] = _('Calculated Return');
		$pg->y[] = abs($transportper);
		$pg->z[] = abs($transportacc);
	}
	$rep->Font();
	$rep->NewLine();
	$rep->Line($rep->row);
