...
function getTaxTransactions($from, $to)
{
	...
	$sql = "SELECT tt.name as taxname, taxrec.*, taxrec.amount*ex_rate AS amount,
		...
		LEFT JOIN ".TB_PREF."consignor_trans dtrans
			ON taxrec.trans_no=dtrans.trans_no AND taxrec.trans_type=dtrans.type
		LEFT JOIN ".TB_PREF."consignors_master as debt ON dtrans.consignor_no=debt.consignor_no
		LEFT JOIN ".TB_PREF."consignor_branch as branch ON dtrans.branch_code=branch.branch_code
		WHERE (taxrec.amount <> 0 OR taxrec.net_amount <> 0)
			AND taxrec.trans_type <> ".ST_CONSIGNORDELIVERY."
...
function print_tax_report()
{
	...
	{
		if (in_array($trans['trans_type'], array(ST_CONSIGNORCREDIT,ST_SUPPINVOICE,ST_JOURNAL))) {
			$trans['net_amount'] *= -1;
			$trans['amount'] *= -1;
		}
		...
		elseif (in_array($trans['trans_type'], array(ST_BANKDEPOSIT,ST_TRANSPORTINVOICE,ST_CONSIGNORCREDIT))) {
			$taxes[$trans['tax_type_id']]['taxout'] += $trans['amount'];
			$taxes[$trans['tax_type_id']]['out'] += $trans['net_amount'];
		} else {
			$taxes[$trans['tax_type_id']]['taxin'] += $trans['amount'];
