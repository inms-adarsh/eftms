...
if (list_updated('PersonDetailID')) {
	...
	$_POST['person_id'] = $br['consignor_no'];
...
function create_cart($type, $trans_no)
{
	...
	if ($trans_no) {
		...
		if ($bank_trans["person_type_id"] == PT_CONSIGNOR)
		{
			$trans = get_consignor_trans($trans_no, $type);	
			$_POST['person_id'] = $trans["consignor_no"];
...
function check_trans()
{
	...
	if (get_post('PayType')==PT_CONSIGNOR && (!get_post('person_id') || !get_post('PersonDetailID'))) {
		display_error(_("You have to select consignor and consignor branch."));
		set_focus('person_id');
		$input_error = 1;
	...
	if (in_array(get_post('PayType'), array(PT_SUPPLIER, PT_CONSIGNOR)) && (input_num('settled_amount') <= 0)) {
