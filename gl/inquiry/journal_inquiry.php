...
//	4=> Funds Transfer,
   ST_TRANSPORTINVOICE => "/transport/consignor_invoice.php?ModifyInvoice=%d",
//   11=>
// free hand (consignors_trans.order_==0)
//	"/transport/credit_note_entry.php?ModifyCredit=%d"
// credit invoice
//	"/transport/consignor_credit_invoice.php?ModifyCredit=%d"
//	 12=> Consignor Payment,
   ST_CONSIGNORDELIVERY => "/transport/consignor_delivery.php?ModifyDelivery=%d",
//   16=> Location Transfer,
//   17=> Inventory Adjustment,
//   20=> Vendor Invoice,
//   21=> Vendor Credit Note,
//   22=> Vendor Payment,
...
function edit_link($row)
{
	...
	{
		$myrow = get_consignor_trans($row["type_no"], $row["type"]);
		if ($myrow['alloc'] != 0 || get_voided_entry(ST_TRANSPORTINVOICE, $row["type_no"]) !== false)
			$ok = false;
	}		
