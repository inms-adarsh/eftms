...
***********************************************************************/
$page_security = 'SA_TRANSPORTKIT';
$path_to_root = "../..";
include_once($path_to_root . "/includes/session.inc");

page(_($help_context = "Transport Kits & Alias Codes"));
...
check_db_has_transportable_items(_("There are no items defined in the system."));
...
{
	$result = get_transport_item_kit($selected_kit);
	div_start('bom');
...
function update_component($kit_code, $selected_item)
{
	...
	elseif ($selected_item == -1)	// adding new item or new alias/kit
	{
		if (get_post('item_code') == '') { // New kit/alias definition
			$kit = get_transport_item_kit($_POST['kit_code']);
    		if (db_num_rows($kit)) {
			  	$input_error = 1;
	...
	if (check_transport_item_in_kit($selected_item, $kit_code, $_POST['component'], true)) {
	...
	if (check_transport_item_in_kit($selected_item, $kit_code, $_POST['component'])) {
	...
	if ($selected_item == -1) { // new item alias/kit
		...
		add_transport_item_code( $kit_code, get_post('component'), get_post('description'), input_num('quantity'), 0,'Transport Kits');
		display_notification($msg);

	} else {
		$props = get_transport_item_kit_props($_POST['item_code']);
		update_transport_item_code($selected_item, $kit_code, get_post('component'),
			$props['description'],  input_num('quantity'), 0);
...
if (get_post('update_name')) {
	update_transport_item_kit_props(get_post('item_code'), get_post('description'));
...
if ($Mode == 'Delete')
{
	...
	// 
	$other_kits = get_kit_where_used($_POST['item_code']);
	$num_kits = db_num_rows($other_kits);

	$kit = get_transport_item_kit($_POST['item_code']);
	if ((db_num_rows($kit) == 1) && $num_kits) {
		...
		delete_transport_item_code($selected_id);
...
echo transport_kits_list('item_code', null, _('New kit'), true);
echo "</center><br>";
$props = get_transport_item_kit_props($_POST['item_code']);
...
} else
{
	...
	stock_categories_transport_list_row(_("Category:"), 'category', null);
	...
	if ($Mode == 'Edit') {
		$myrow = get_transport_item_code($selected_id);
		$_POST['component'] = $myrow["stock_id"];
		$_POST['quantity'] = number_format2($myrow["quantity"], get_qty_dec($myrow["stock_id"]));
	}
	hidden("selected_id", $selected_id);
	
	transport_local_items_list_row(_("Component:"),'component', null, false, true);
