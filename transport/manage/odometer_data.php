...
if (!@$_GET['popup'])
	page(_($help_context = "Odometer Data"));
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM')
{
	...
	if ($input_error == 0)
	{
     	if ($Mode == 'ADD_ITEM') 
       	{
			add_vehicle_odometer_data ($_POST['vehicle_id'],$_POST['Location'], input_num('odometer_reading'),
	 $_POST['date_of_reading'], $_POST['driver_id'], $_POST['log_description']);
    		display_notification(_("This odometer data has been added."));
       	} 
       	else
       	{
       		update_vehicle_odometer_data($selected_id, $_POST['vehicle_id'],$_POST['Location'], input_num('odometer_reading'),
	 $_POST['date_of_reading'], $_POST['driver_id'], $_POST['log_description']);
...
if ($Mode == 'Delete')
{
	delete_vehicle_odometer_data($selected_id, $_POST['vehicle_id']);
...
if (!@$_GET['popup'])
{
	echo "<div class='form-group'><label class='control-label col-md-3'>" . _("Vehicle:"). "&nbsp;</label><div class='col-md-6'>";
	...
	$result = get_vehicle_odometer_data($_POST['vehicle_id']);
    ...
    start_display_table('table table-striped table-bordered','id');
    start_row();
		$max_reading = get_max_odometer_reading($_POST['vehicle_id']);
		 $max_reading_to_display = $max_reading["max_reading"];
		label_row(_("Max Reading"),$max_reading_to_display);
        ...
        while ($myrow = db_fetch($result))
        {
           ...
           label_cell($myrow["log_description"]);
		 	edit_button_cell("Edit".$myrow['odometer_id'], _("Edit"));
		 	delete_button_cell("Delete".$myrow['odometer_id'], _("Delete"));
            end_row();
...
if ($Mode =='Edit')
{
	...
	$myrow = get_vehicle_odometer_datas($selected_id, $_POST['vehicle_id']);
