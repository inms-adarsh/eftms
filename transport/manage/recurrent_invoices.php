...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM') 
{
	...
	if ($input_error != 1)
	{
    	if ($selected_id != -1) 
    	{
    		update_recurrent_invoice($selected_id, $_POST['description'], $_POST['order_no'], input_num('consignor_no'), 
    	...
    	{
    		add_recurrent_invoice($_POST['description'], $_POST['order_no'], input_num('consignor_no'), input_num('group_no'),
...
while ($myrow = db_fetch($result)) 
{
	...
	label_cell(get_consignor_trans_view_str(ST_TRANSPORTORDER, $myrow["order_no"]));
...
if ($selected_id != -1) 
{
 	if ($Mode == 'Edit') {
		...
		$_POST['consignor_no']  = $myrow["consignor_no"];
...
consignor_list_row(_("Consignor:"), 'consignor_no', null, " ", true);

if ($_POST['consignor_no'] > 0)
	consignor_branches_list_row(_("Branch:"), $_POST['consignor_no'], 'group_no', null, false);
else	
	transport_groups_list_row(_("Transport Group:"), 'group_no', null, " ");
