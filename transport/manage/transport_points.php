...
include_once($path_to_root . "/transport/includes/db/transport_points_db.inc");
...
{
	add_transport_point($_POST['name'], $_POST['location'], $_POST['account'],
...
if ($Mode=='UPDATE_ITEM' && can_process())
{
	...
	update_transport_point($selected_id, $_POST['name'], $_POST['location'],
...
if ($Mode == 'Delete')
{
	if (key_in_foreign_table($selected_id, 'users', 'pos'))
	{
		...
		delete_transport_point($selected_id);
...
$result = get_all_transport_points(check_value('show_inactive'));
...
while ($myrow = db_fetch($result))
{
	...
	inactive_control_cell($myrow["id"], $myrow["inactive"], "transport_pos", 'id');
...
if ($selected_id != -1)
{
 	...
 	if ($Mode == 'Edit') {
		$myrow = get_transport_point($selected_id);
