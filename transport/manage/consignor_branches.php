...
page(_($help_context = "Consignor Branches"), @$_REQUEST['popup']);
...
if ($Mode=='ADD_ITEM' || $Mode=='UPDATE_ITEM')
{
	...
	if ($input_error != 1)
	{
    	...
    	if ($selected_id != -1)
		{
			update_branch($_POST['consignor_id'], $_POST['branch_code'], $_POST['br_name'], $_POST['tax_group_id'], $_POST['default_location'],
			$_POST['br_ref'],$_POST['tin_no'],
...
//				'consignor_branch', 'branch_code');
		...
		{
			add_branch($_POST['consignor_id'], $_POST['br_name'], $_POST['tax_group_id'],$_POST['default_location'],  $_POST['br_ref'],
				 $_POST['tin_no'],$_POST['br_address'],
...
elseif ($Mode == 'Delete')
{
	...
	if (branch_in_foreign_table($_POST['consignor_id'], $_POST['branch_code'], 'consignor_trans'))
	{
	...
	{
		if (branch_in_foreign_table($_POST['consignor_id'], $_POST['branch_code'], 'transport_orders'))
		{
			display_error(_("Cannot delete this branch because transport orders exist for it. Purge old transport orders first."));
		}
		else
		{
			delete_branch($_POST['consignor_id'], $_POST['branch_code']);
...
if ($Mode == 'RESET' || get_post('_consignor_id_update'))
{
	...
	$_POST['consignor_id'] = $consignor_id;
...
//		    $_POST['email'] =$myrow["email"];
	...
	{ //end of if $SelectedBranch only do the else when a new record is being entered
		$myrow = get_default_info_for_branch($_POST['consignor_id']);
//		$_POST['rep_lang'] = $myrow['rep_lang'];
		if(!$num_branches) {
	...
	label_row(_("Search Address"),"<input id='autocomplete' style='width:300px' placeholder='Enter your address' onFocus='geolocate()' type='text'></input>");

	text_row(_("Branch Name:"), 'br_name', null, 35, 40,null,null,null,'id=locality');
	text_row(_("Branch Short Name:"), 'br_ref', null, 30, 30,null,null,null,'id=short_locality');
...
locations_list_row(_("Default Inventory Location:"), 'default_location', null);
	...
	transport_groups_list_row(_("Transport Group:"), 'group_no', null, true);
	locations_list_row(_("Default Inventory Location:"), 'default_location', null);
	shippers_list_row(_("Default Shipping Company:"), 'default_ship_via', null);
	

	table_section_title(_("GL Accounts"));
	// 2006-06-14. Changed gl_al_accounts_list to have an optional all_option 'Use Item Transport Accounts'
	gl_all_accounts_list_row(_("Transport Account:"), 'transport_account', null, false, false, true);
	gl_all_accounts_list_row(_("Transport Discount Account:"), 'transport_discount_account');
...
if($selected_id==-1) {
	...
	languages_list_row( _("Document Language:"), 'rep_lang', null, _("Consignor default"));
}
	table_section_title(_("Addresses"));
	textarea_row(_("Mailing Address:"), 'br_post_address', null, 35, 5);
	textarea_row(_("Billing Address:"), 'br_address', null, 35, 5,null,null,'address');
...
$num_branches = db_consignor_has_branches($_POST['consignor_id']);
...
echo "<script src='$path_to_root/themes/fa-boot/js/geolocation.js'></script>";
