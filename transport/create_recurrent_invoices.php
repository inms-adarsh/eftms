...
function create_recurrent_invoices($consignor_id, $branch_id, $order_no, $tmpl_no, $date, $from, $to)
{
	global $Refs;

	$doc = new Cart(ST_TRANSPORTORDER, array($order_no));

	get_consignor_details_to_order($doc, $consignor_id, $branch_id);

	$doc->trans_type = ST_TRANSPORTORDER;
	$doc->trans_no = 0;
	...
	foreach ($doc->line_items as $line_no=>$item) {
		...
		$line->price = get_price($line->stock_id, $doc->consignor_currency,
			$doc->transport_type, $doc->price_factor, $doc->document_date);
	}	
	$cart = $doc;
	$cart->trans_type = ST_TRANSPORTINVOICE;
...
if ($id != -1)
{
	...
	if (is_date_in_fiscalyear($date))
	{
		...
		if ($myrow['consignor_no'] == 0)
		{
			$consignor = get_consignor_branches_from_group($myrow['group_no']);
			while ($row = db_fetch($consignor))
			{
				$invs[] = create_recurrent_invoices($row['consignor_no'], $row['branch_code'], $myrow['order_no'], $myrow['id'],
					$date, $from, $to);
			}	
		}
		else
		{
			$invs[] = create_recurrent_invoices($myrow['consignor_no'], $myrow['group_no'], $myrow['order_no'], $myrow['id'],
				$date, $from, $to);
		}
		if (count($invs) > 0)
		{
		...
		if (count($invs) > 0)
		{
			$ar = array('PARAM_0' => $min."-".ST_TRANSPORTINVOICE,	'PARAM_1' => $max."-".ST_TRANSPORTINVOICE, 'PARAM_2' => "",
...
$th = array(_("Description"), _("Template No"),_("Consignor"),_("Branch")."/"._("Group"),_("Days"),_("Monthly"),_("Begin"),_("End"),_("Last Created"),"");
table_header($th);
$k = 0;
...
while ($myrow = db_fetch($result)) 
{
	...
	label_cell(get_consignor_trans_view_str(30, $myrow["order_no"]));
	if ($myrow["consignor_no"] == 0)
	{
		label_cell("");
		label_cell(get_transport_group_name($myrow["group_no"]));
	}	
	else
	{
		label_cell(get_consignor_name($myrow["consignor_no"]));
