...
page(_($help_context = "View Transport Invoice"), true, false, "", $js);
...
display_heading(sprintf(_("TRANSPORT INVOICE #%d"),$trans_id));
...
label_cells(_("Deliveries"), get_consignor_trans_view_str(ST_CONSIGNORDELIVERY, 
	get_transport_parent_numbers(ST_TRANSPORTINVOICE, $trans_id)), "class='tableheader2'");
end_row();
comments_display_row(ST_TRANSPORTINVOICE, $trans_id);
end_table();
...
if (db_num_rows($result) > 0)
{
	...
	while ($myrow2 = db_fetch($result))
	{
        ...
        label_cell($myrow2["to_units"], "align=right");
...
label_row(_("Shipping + Service Tax"), $display_freight, "colspan=6 align=right", "nowrap align=right");

$tax_items = get_trans_tax_details(ST_TRANSPORTORDER, $trans_id);
...
$voided = is_voided_display(ST_TRANSPORTORDER, $trans_id, _("This invoice has been voided."));
/*
if (!$voided)
{
	display_allocations_to(PT_CONSIGNOR, $myrow['consignor_no'], ST_TRANSPORTINVOICE, $trans_id, $myrow['Total']);
}*/

end_page(true, false, false, ST_TRANSPORTORDER, $trans_id);
