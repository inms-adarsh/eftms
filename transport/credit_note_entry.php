...
//
$page_security = 'SA_TRANSPORTCREDIT';
$path_to_root = "..";
...
include_once($path_to_root . "/transport/includes/ui/transport_credit_ui.inc");
...
if(isset($_GET['NewCredit'])) {
	$_SESSION['page_title'] = _($help_context = "Consignor Credit Note");
	handle_new_credit(0);
} elseif (isset($_GET['ModifyCredit'])) {
	$_SESSION['page_title'] = sprintf(_("Modifying Consignor Credit Note #%d"), $_GET['ModifyCredit']);
	handle_new_credit($_GET['ModifyCredit']);
	$help_context = "Modifying Consignor Credit Note";
}
...
if (isset($_GET['AddedID'])) {
	...
	$trans_type = ST_CONSIGNORCREDIT;

	display_notification_centered(sprintf(_("Credit Note # %d has been processed"),$credit_no));

	display_note(get_consignor_trans_view_str($trans_type, $credit_no, _("&View this credit note")), 0, 1);

	display_note(print_document_link($credit_no."-".$trans_type, _("&Print This Credit Invoice"), true, ST_CONSIGNORCREDIT),0, 1);
	display_note(print_document_link($credit_no."-".$trans_type, _("&Email This Credit Invoice"), true, ST_CONSIGNORCREDIT, false, "printlink", "", 1),0, 1);
...
function copy_to_cn()
{
	...
	$cart->transport_type = $_POST['transport_type_id'];
	if ($cart->trans_no == 0)
...
function copy_from_cn()
{
	...
	$_POST['transport_type_id'] = $cart->transport_type;
	if ($cart->trans_no == 0)
...
{
	processing_start();
	$_SESSION['Items'] = new Cart(ST_CONSIGNORCREDIT,$trans_no);
	copy_from_cn();
}
...
function handle_new_item()
{
	...
	add_to_order($_SESSION['Items'], $_POST['stock_id'], input_num('qty'),input_num('to_units'),
		input_num('price'), input_num('Disc') / 100);
    line_start_focus();
}
//-----------------------------------------------------------------------------
...
$consignor_error = display_credit_header($_SESSION['Items']);
