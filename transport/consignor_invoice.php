...
//---------------------------------------------------------------------------
//
//	Entry/Modify Transport Invoice against single delivery
//	Entry/Modify Batch Transport Invoice against batch of deliveries
//
...
if (isset($_GET['ModifyInvoice'])) {
	$_SESSION['page_title'] = sprintf(_("Modifying Transport Invoice # %d.") ,$_GET['ModifyInvoice']);
	$help_context = "Modifying Transport Invoice";
...
if (isset($_GET['AddedID'])) {
	...
	$trans_type = ST_TRANSPORTINVOICE;

	display_notification(_("Selected deliveries has been processed"), true);

	display_note(get_consignor_trans_view_str($trans_type, $invoice_no, _("&View This Invoice")), 0, 1);

	display_note(print_document_link($invoice_no."-".$trans_type, _("&Print This Invoice"), true, ST_TRANSPORTINVOICE));
	display_note(print_document_link($invoice_no."-".$trans_type, _("&Email This Invoice"), true, ST_TRANSPORTINVOICE, false, "printlink", "", 1),1);

	display_note(get_gl_view_str($trans_type, $invoice_no, _("View the GL &Journal Entries for this Invoice")),1);

	hyperlink_params("$path_to_root/transport/inquiry/transport_deliveries_view.php", _("Select Another &Delivery For Invoicing"), "OutstandingOnly=1");
	...
	display_notification_centered(sprintf(_('Transport Invoice # %d has been updated.'),$invoice_no));

	display_note(get_trans_view_str(ST_TRANSPORTINVOICE, $invoice_no, _("&View This Invoice")));
	echo '<br>';
	display_note(print_document_link($invoice_no."-".$trans_type, _("&Print This Invoice"), true, ST_TRANSPORTINVOICE));
	display_note(print_document_link($invoice_no."-".$trans_type, _("&Email This Invoice"), true, ST_TRANSPORTINVOICE, false, "printlink", "", 1),1);

	hyperlink_no_params($path_to_root . "/transport/inquiry/consignor_inquiry.php", _("Select Another &Invoice to Modify"));
...
|| isset($_GET['BatchInvoice'])) {
	...
	$dn = new Cart(ST_CONSIGNORDELIVERY, $src, true);

	if ($dn->count_items() == 0) {
		hyperlink_params($path_to_root . "/transport/inquiry/transport_deliveries_view.php",
	...
	if ( get_transport_parent_numbers(ST_TRANSPORTINVOICE, $_GET['ModifyInvoice']) == 0) { // 1.xx compatibility hack
	...
	processing_start();
	$_SESSION['Items'] = new Cart(ST_TRANSPORTINVOICE, $_GET['ModifyInvoice']);
	...
	hyperlink_no_params("$path_to_root/transport/inquiry/transport_deliveries_view.php", _("Select Delivery to Invoice"));
...
function set_delivery_shipping_sum($delivery_notes) 
{
    ...
    {
        $myrow = get_consignor_trans($delivery_num, 13);
        //$branch = get_branch($myrow["branch_code"]);
        //$transport_order = get_transport_order_header($myrow["order_"]);
        
        //$shipping += $transport_order['freight_cost'];
...
$is_edition = $_SESSION['Items']->trans_type == ST_TRANSPORTINVOICE && $_SESSION['Items']->trans_no != 0;
start_form();
hidden('cart_id');
...
//get_consignor_trans_view_str(ST_CONSIGNORDELIVERY, array_keys($_SESSION['Items']->src_docs)), "class='tableheader2'");
...
It seems unfair to charge the consignor twice for freight if the order
