...
//-----------------------------------------------------------------------------
//
//	Entry/Modify Delivery Note against Transport Booking
//
$page_security = 'SA_TRANSPORTDELIVERY';
$path_to_root = "..";
...
if (isset($_GET['ModifyDelivery'])) {
	...
	$_SESSION['page_title'] = _($help_context = "Deliver Items for a Bilty");
	processing_start();
}
...
if (isset($_GET['AddedID'])) {
	...
	display_note(get_consignor_trans_view_str(ST_CONSIGNORDELIVERY, $dispatch_no, _("&View This Delivery")), 0, 1);

	display_note(print_document_link($dispatch_no, _("&Print Delivery Note"), true, ST_CONSIGNORDELIVERY));
	display_note(print_document_link($dispatch_no, _("&Email Delivery Note"), true, ST_CONSIGNORDELIVERY, false, "printlink", "", 1), 1, 1);
	display_note(print_document_link($dispatch_no, _("P&rint as Packing Slip"), true, ST_CONSIGNORDELIVERY, false, "printlink", "", 0, 1));
	display_note(print_document_link($dispatch_no, _("E&mail as Packing Slip"), true, ST_CONSIGNORDELIVERY, false, "printlink", "", 1, 1), 1);

	display_note(get_gl_view_str(13, $dispatch_no, _("View the GL Journal Entries for this Dispatch")),1);

	hyperlink_params("$path_to_root/transport/consignor_invoice.php", _("Invoice This Delivery"), "DeliveryNumber=$dispatch_no");

	hyperlink_params("$path_to_root/transport/inquiry/transport_orders_view.php", _("Select Another Bilty Items For Dispatch"), "OutstandingOnly=1");
	...
	display_note(get_trans_view_str(ST_CONSIGNORDELIVERY, $delivery_no, _("View this delivery")), 0, 1);

	display_note(print_document_link($delivery_no, _("&Print Delivery Note"), true, ST_CONSIGNORDELIVERY));
	display_note(print_document_link($delivery_no, _("&Email Delivery Note"), true, ST_CONSIGNORDELIVERY, false, "printlink", "", 1), 1, 1);
	display_note(print_document_link($delivery_no, _("P&rint as Packing Slip"), true, ST_CONSIGNORDELIVERY, false, "printlink", "", 0, 1));
	display_note(print_document_link($delivery_no, _("E&mail as Packing Slip"), true, ST_CONSIGNORDELIVERY, false, "printlink", "", 1, 1), 1);

	hyperlink_params($path_to_root . "/transport/consignor_invoice.php", _("Confirm Delivery and Invoice"), "DeliveryNumber=$delivery_no");

	hyperlink_params($path_to_root . "/transport/inquiry/transport_deliveries_view.php", _("Select A Different Delivery"), "OutstandingOnly=1");
...
*/
	...
	$_SESSION['Items'] = new Cart(ST_CONSIGNORDELIVERY, $_GET['ModifyDelivery']);
	...
	display_error(_("This page can only be opened if an bilty or delivery note has been selected. Please select it first."));

	hyperlink_params("$path_to_root/transport/inquiry/transport_orders_view.php", _("Select a Transport Booking to Delivery"), "OutstandingOnly=1");
	...
	if (!check_quantities()) {
		display_error(_("Selected quantity cannot be less than quantity invoiced nor more than quantity	not dispatched on transport booking."));
...
start_row();
label_cells(_("Consignor"), $_SESSION['Items']->consignor_name, "class='tableheader2'");
label_cells(_("Branch"), get_branch_name($_SESSION['Items']->Branch), "class='tableheader2'");
label_cells(_("Currency"), $_SESSION['Items']->consignor_currency, "class='tableheader2'");
end_row();
start_row();

//if (!isset($_POST['ref']))
//	$_POST['ref'] = $Refs->get_next(ST_CONSIGNORDELIVERY);
...
label_cells(_("For Bilty"), get_consignor_trans_view_str(ST_TRANSPORTORDER, $_SESSION['Items']->order_no), "class='tableheader2'");

label_cells(_("Transport Type"), $_SESSION['Items']->transport_type_name, "class='tableheader2'");
end_row();
start_row();
...
consignor_credit_row($_SESSION['Items']->consignor_id, $_SESSION['Items']->credit, "class='tableheader2'");
// 2010-09-03 Joe Hunt
...
$row = get_consignor_to_order($_SESSION['Items']->consignor_id);
if ($row['dissallow_invoices'] == 1)
{
	display_error(_("The selected consignor account is currently on hold. Please contact the credit control personnel to discuss."));
	end_form();
	end_page();
	exit();
}	
...
$taxes = $_SESSION['Items']->get_taxes($inv_items_total);
