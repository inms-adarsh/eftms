...
page(_($help_context = "Vendors"), @$_REQUEST['popup'], false, "", $js);
...
function supplier_settings(&$supplier_id)
{
	...
	if ($supplier_id) 
	{
		//VendorID exists - either passed when calling the form or from the form itself
	...
	text_row(_("Vendor Name:"), 'supp_name', null, 42, 40);
	text_row(_("Vendor Short Name:"), 'supp_ref', null, 30, 30);

	text_row(_("GSTNo:"), 'gst_no', null, 42, 40);
	link_row(_("Website:"), 'website', null, 35, 55);
	if ($supplier_id && !is_new_supplier($supplier_id)) 
	{
		label_row(_("Vendor's Currency:"), $_POST['curr_code']);
		hidden('curr_code', $_POST['curr_code']);
	} 
	else 
	{
		currencies_list_row(_("Vendor's Currency:"), 'curr_code', null);
	}
	tax_groups_list_row(_("Tax Group:"), 'tax_group_id', null);
	text_row(_("Our Consignor No:"), 'supp_account_no', null, 42, 40);
	...
	if ($supplier_id) 
	{
		submit_center_first('submit', _("Update Vendor"), 
		...
		submit_center_last('delete', _("Delete Vendor"), 
		  _('Delete supplier data if have been never used'), true);
	}
	else 
	{
		submit_center('submit', _("Add New Vendor Details"), true, '', 'default');
	}
	div_end();
}
...
if (isset($_POST['submit'])) 
{
	...
	if ($input_error !=1 )
	{
		...
		if ($supplier_id) 
		{
			...
			display_notification(_("Vendor has been updated."));
		} 
		else 
		{
